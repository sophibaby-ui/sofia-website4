...
/* 農曆換算改用 lunar-javascript 套件，避免自製對照表遺漏「閏月不遞增月份」造成閏月之後全部錯位一個月的問題 */
function solarToLunar(sy, sm, sd) {
  const lunar = Solar.fromYmd(sy, sm, sd).getLunar();
  const rawMonth = lunar.getMonth(); // 閏月時 lunar-javascript 會回傳負數，例如閏二月 = -2
  return {
    y: lunar.getYear(),
    m: Math.abs(rawMonth),
    d: lunar.getDay(),
              ...
              <div style={{ display: "flex", gap: "14px", flexWrap: "wrap", marginBottom: "34px" }}>
                ...
                {result.lunarDate && (
                  <div style={{ flex: "1 1 240px", padding: "20px 24px", borderRadius: "18px", background: "#FFFDF8", border: "1px solid rgba(160,200,205,.5)" }}>
                    ...
                    <div style={{ fontFamily: "'Noto Serif TC',serif", fontSize: "18px", color: "#2A2723" }}>農曆 {result.lunarDate.y} 年 {result.lunarDate.isLeap ? "閏" : ""}{result.lunarDate.m} 月 {result.lunarDate.d} 日</div>
                  </div>
                )}
              </div>
